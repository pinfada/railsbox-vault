...
test("la TABLE du § 4.5 est celle que la session produit, poste par poste", () => {
  ...
  // Trois pages d'enveloppe ont été écrites — créer, ajouter, révoquer —, une page de récupération,
  // une capture et une archive. Chacune sous SA clé, et une seule fois sous elle.
  assert.equal(parDomaine.get(DOMAINES.enveloppe).cles, 3, "trois pages d'enveloppe écrites");
  assert.equal(parDomaine.get(DOMAINES.enveloppe).invocations, 3, "une invocation par page");
