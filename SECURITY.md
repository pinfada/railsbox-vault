...
- `SEC-KEY-001` — une clé de déverrouillage enveloppe une DEK aléatoire sans servir directement au
  ...
  format v4 construise une clé AES-GCM depuis une clé de volume, et il MORD : douze mutants, douze
  tués. **CE QUI RESTE, et il est écrit partout de la même phrase** : ouvrir un volume **v3** — pour
  le migrer, ou pour l'EXPORTER avant de le migrer — fait écrire au magasin une racine v3 et
  rescelle la charge acquittée, c'est-à-dire **3 + N scellements** sous la clé de volume elle-même —
  l'empreinte de région, la racine de clôture et le témoin, plus un par secteur rejoué de la charge
  acquittée. Ce nombre est MESURÉ et non arrondi : `tests/unit/vm-migration-source-v3.test.mjs`
  compte 3 sans rien à rejouer, 4 avec une écriture acquittée restée au journal, et vérifie que la
  racine v3 publie exactement ce qu'elle a consommé. N n'est pas borné par ce document : il l'est
  par le contenu du journal validé. C'est le régime que la v4 remplace, c'est l'unique exception du
  cliquet, et ne pas ouvrir perdrait une écriture acquittée. Décisions :
