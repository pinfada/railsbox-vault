...
| `SEC-ORIGIN-001` — coquille de PRODUIT | **exercé sous réserve** | la frontière est éprouvée sur les ports RÉELS de la coquille de produit ; le geste de DÉVERROUILLAGE est celui d'un UTILISATEUR depuis #162 — phrase, passkey ou code, aucun jeton de harnais sur le chemin de produit — ; ce qui reste réservé est le cycle de vie non assemblé (#163) | `tests/browser/coquille-frontiere.spec.mjs` (application malveillante, dix gestes refusés, témoin positif en même origine, trois moteurs), `tests/browser/coquille-deverrouillage.spec.mjs` (les trois moyens depuis la coquille, l'attente annoncée, la feuille, la sonde étendue, trois moteurs), `tests/unit/coquille-admission.test.mjs`, `tests/unit/coquille-contrat.test.mjs`, `tests/unit/coquille-deverrouillage.test.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
2. **Données sensibles interdites** jusqu'à l'implémentation de la séparation d'origine, du
   ...
   **implémentée depuis #161** — une coquille de produit, un Worker de confiance, un port restreint
   ...
   livrés : c'est un compte, pas une autorisation.
