...
- `SEC-KEY-001` — une clé de déverrouillage enveloppe une DEK aléatoire sans servir directement au
  ...
  ([#182](https://github.com/pinfada/railsbox-vault/issues/182), HIGH, **en cours : T2a livré, T2b
  reste**). La DEK est désormais une clé MAÎTRESSE : elle est importée en matériau HKDF, et
  `crypto.subtle.encrypt` la REJETTE par la spécification WebCrypto — ce n'est plus une discipline,
  ...
  **Depuis #169, la DURÉE de vie de cette clé dans le Worker de confiance est une propriété ÉPROUVÉE
  et non plus une promesse** : un geste « Verrouiller » et un délai d'inactivité de dix minutes
  tuent le Worker qui la détient, après la fermeture propre des volumes. **Ce qui est mesuré, et
  où** : le GESTE et le verrouillage REFUSÉ sur les trois moteurs, le DÉLAI en temps réel sur
  Chromium et Firefox — WebKit n'ouvre rien, donc n'arme rien, et la suite le DÉCLARE
...
- `SEC-BLOCK-001` — un bloc est authentifié avec volume, adresse, format, génération **et magasin**.
  **Ces quatre champs séparent des IDENTITÉS LOGIQUES, et depuis le 11 septembre 2026 les CLÉS sont
  ...
  ([#182](https://github.com/pinfada/railsbox-vault/issues/182), HIGH, en cours) : « les données
  associées séparent les domaines logiques, mais ne protègent pas AES-GCM contre la réutilisation du
  même couple clé/nonce ». Le format v4 donne à chaque domaine de chaque volume sa propre clé
  dérivée : le RAYON d'une collision de nonce est désormais un (domaine × volume), au lieu de tout
...
| `SEC-KEY-001`                          | **exercé sous réserve** | RÉSERVE RÉDUITE le 11 septembre 2026 (#182, tranche T2a) : la DEK est désormais une clé maîtresse importée en matériau HKDF, que WebCrypto refuse de passer à AES-GCM, et quatre domaines en descendent — `volume`, `journal`, `instantane`, `archive`. Deux volumes sous la même DEK ne partagent plus aucune clé. CE QUI RESTE, et c'est nommé plutôt qu'arrondi : la page d'enveloppe et la section de récupération scellent encore sous la DEK (tranche T2b), le cliquet anti-DEK définitif n'est pas posé — celui de T2a est un INVENTAIRE, qui relève la porte nommée et toute importation directe d'une clé AES-GCM, sur `src/` et `public/` —, et **le volume de COQUILLE scelle un secteur par déverrouillage hors clôture, non compté, jusqu'à T2b**. Cet écart-là n'a pas de borne : il croît d'un scellement par déverrouillage, et il est MESURÉ par `tests/unit/vm-cloture-par-racine.test.mjs` › « CHEMIN 3 » au lieu d'être gardé par une règle. La DURÉE de rétention de la KEK dans le Worker de confiance est BORNÉE depuis #169 (geste « Verrouiller » et délai d'inactivité de dix minutes), et elle est mesurée ; ce que le verrouillage ne couvre pas est écrit avec lui — mémoire du processus, fichier d'échange, copie déjà prise | `tests/browser/enveloppe-frontiere.spec.mjs`, `tests/browser/coquille-cycle-de-vie.spec.mjs` (le verrouillage par geste et par délai, trois moteurs), `tests/unit/coquille-verrouillage.test.mjs` (l'état, la règle et les bornes du délai)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `SEC-BLOCK-001`                        | **exercé sous réserve** | le format scelle tout, de bout en bout — mais la clé du chemin de BOOT vient encore du harnais, sous jeton. Le constat du 10 septembre 2026 (#182) sur les données associées est CORRIGÉ pour les domaines du volume depuis le 11 septembre : chaque domaine de chaque volume scelle sous sa propre clé dérivée. Le budget annoncé est celui que le mécanisme tient **sur les chemins que la v4 ferme** — création, installation initiale et sessions transactionnelles, mesurés à l'ÉGALITÉ par le nombre d'invocations réelles —, et DEUX sous-estimations subsistent, écrites au § 4.5 de la spécification : le volume de coquille scelle un secteur par déverrouillage hors clôture, non compté, jusqu'à T2b (sans borne) ; une migration reprise peut rescéller une suite deux fois (au plus 512 secteurs par coupure)                                                                                                                                                                                                                                                                                                                                                                                                                                 | `tests/unit/vm-volume-chiffre.test.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
