...
`sandbox="allow-scripts allow-same-origin allow-forms"` — le troisième jeton depuis #192 (ADR 0038,
décision 4 bis), qui n'ajoute que la soumission d'un formulaire : ni popup, ni navigation du sommet,
ni modale, ni téléchargement.
...
cette barrière survit au verrouillage, à la sauvegarde et au boot à froid. **Persistance d'une
écriture acquittée (#209)** : une écriture que Rails a acquittée — ligne SQLite et pièce jointe
ActiveStorage de l'application de référence — est durable dès l'acquittement : le commit
(`synchronous = extra`) et le téléversement (`DurableDisk`) ne rendent la main qu'après des
...
- `SEC-KEY-001` — une clé de déverrouillage enveloppe une DEK aléatoire sans servir directement au
  chiffrement des blocs. **La hiérarchie compte un étage de plus depuis le 11 septembre 2026**, et
  c'est la revue externe du 10 septembre qui a montré qu'il manquait
  ([#182](https://github.com/pinfada/railsbox-vault/issues/182), HIGH, **corrigé le 11 septembre
  2026**). La DEK est une clé MAÎTRESSE : elle est importée en matériau HKDF, et
  `crypto.subtle.encrypt` la REJETTE par la spécification WebCrypto — ce n'est pas une discipline,
  c'est la plate-forme qui refuse. **Les SIX domaines** en descendent par HKDF-SHA-256, propres au
  domaine, au volume et à la version de format : `volume`, `journal`, `instantane`, `enveloppe`,
  `archive` et `recuperation`. Deux volumes sous la même DEK ne partagent aucune clé, et le compteur
  d'une clé compte toutes les invocations sous elle — sur les TROIS chemins hors transaction comme
  sur les sessions transactionnelles. **C'est MESURÉ, et non affirmé** :
  `tests/unit/vm-budget-par-domaine.test.mjs` étiquette chaque `CryptoKey` par sa provenance et
  compte les invocations de `encrypt` par clé sur une session complète du produit — zéro sous la clé
  de volume, jamais deux sous une clé à usage unique. Un CLIQUET d'inspection de source
  (`tests/unit/vm-cliquet-anti-dek.test.mjs`) refuse par ailleurs qu'un chemin de production du
  format v4 construise une clé AES-GCM depuis une clé de volume, et il MORD : dix-huit mutants,
  dix-huit tués — dont QUATRE qui portent chacun sur un motif du cliquet pris séparément. **CE QUI
  RESTE, et il est écrit partout de la même phrase** : ouvrir un volume **v3** — pour le migrer, ou
  pour l'EXPORTER avant de le migrer — fait écrire au magasin une racine v3 et rescelle la charge
  acquittée, c'est-à-dire **3 + N scellements** sous la clé de volume elle-même — l'empreinte de
  région, la racine de clôture et le témoin, plus un par secteur rejoué de la charge acquittée. Ce
  nombre est MESURÉ et non arrondi : `tests/unit/vm-migration-source-v3.test.mjs` compte 3 sans rien
  à rejouer, 4 avec une écriture acquittée restée au journal, et vérifie que la racine v3 publie
  exactement ce qu'elle a consommé. N n'est pas borné par ce document : il l'est par le contenu du
  journal validé. C'est le régime que la v4 remplace, c'est l'unique exception du cliquet, et ne pas
  ouvrir perdrait une écriture acquittée. Décisions :
  l'[ADR 0033](docs/decisions/0033-hierarchie-de-cles-derivees-par-domaine.md) (la hiérarchie),
  l'[ADR 0035](docs/decisions/0035-format-de-volume-v4-et-migration.md) (le format v4) et
  l'[ADR 0036](docs/decisions/0036-page-d-enveloppe-v2-et-budgets-exhaustifs.md) (la page
  d'enveloppe v2, la clôture du troisième chemin, le budget mesuré). **Depuis #169, la DURÉE de vie
  de cette clé dans le Worker de confiance est une propriété ÉPROUVÉE et non plus une promesse** :
  un geste « Verrouiller » et un délai d'inactivité de dix minutes tuent le Worker qui la détient,
  après la fermeture propre des volumes. **Ce qui est mesuré, et où** : le GESTE et le verrouillage
  REFUSÉ sur les trois moteurs, le DÉLAI en temps réel sur Chromium et Firefox — WebKit n'ouvre
  rien, donc n'arme rien, et la suite le DÉCLARE
  ([ADR 0031](docs/decisions/0031-verrouiller-le-worker-meurt-l-instantane-survit.md)). Ce que cela
  ne dit pas est écrit avec : ni la mémoire du processus, ni le fichier d'échange ne sont couverts.
  **Depuis #22 un humain obtient réellement sa clé**, et la réserve que #21 portait ici est levée :
  deux dérivateurs ([ADR 0021](docs/decisions/0021-derivation-des-cles-de-deverrouillage.md))
  rendent une KEK `CryptoKey` **non extractible**, dont les octets ne sont atteignables par aucun
  code de cette origine. `phrase` étire la phrase par **Argon2id (RFC 9106)** calculé par un
  artefact WebAssembly VENDU dans le dépôt — implémentation de référence `phc-winner-argon2`,
  version épinglée, empreinte SHA-256 recalculée **avant instanciation** dans le navigateur et
  vérifiée une seconde fois sur l'arbre publié par `publier:check` ; les vecteurs de la RFC sont
  rejoués sur les trois moteurs par le binaire réellement servi. Le **plancher de coût est celui de
  la RFC** — 64 Mio, trois passes, quatre voies — et il est vérifié à l'écriture ET à la lecture des
  paramètres, parce que l'affaiblissement de ces octets par un adversaire qui garde une copie du
  volume est exactement l'attaque que l'ADR 0020 avait nommée en les authentifiant. `webauthn-prf`
  évalue l'extension `prf` sur un sel de trente-deux octets propre à l'emplacement, exige
  `residentKey` et `userVerification`, et **ne lit pas `signCount`** (facultatif en CTAP2, ADR
  0015). Les deux passent par HKDF-SHA-256 dont l'info lie identifiant de volume, identifiant
  d'emplacement et version — sans quoi une passkey enregistrée pour un emplacement ouvrirait le
  voisin, sur un autre volume. **Ce qui est décidé et mesuré quand la plate-forme ne peut pas** :
  PRF absent à l'enregistrement, extension ignorée à l'assertion, annulation — trois codes
  distincts, aucun repli automatique, **aucun compteur d'échec persisté**, et un type d'emplacement
  qu'aucun dérivateur ne sert refusé sans qu'un octet du fichier ne bouge. **Ce que JavaScript ne
  garantit pas est écrit** : les tampons de matériau sont mis à zéro dès que la clé existe, du côté
  JavaScript comme dans le tas WebAssembly — fenêtre refermée, pas garantie, puisque le moteur a pu
  les copier —, la chaîne PHC d'Argon2, qui porterait le matériau en base64, n'est **pas demandée du
  tout** plutôt que demandée puis effacée, et une phrase est une `string`, donc impossible à effacer
  ; aucun code JavaScript ne verrouille non plus une page en mémoire contre un fichier d'échange. Le
  coût est borné des DEUX côtés : sous le plancher un volume volé serait cassable, au-delà du
  plafond un adversaire qui écrit le fichier ferait calculer le Worker de confiance pendant des
  heures à chaque tentative. Vingt-trois gardes ont été RÉELLEMENT mutées, vingt-trois tuées, dont
  cinq seulement après l'écriture de l'épreuve qui manquait — l'une d'elles a révélé que le plancher
  de coût n'était éprouvé qu'à l'écriture. La disposition de l'enveloppe est
...
- `SEC-BLOCK-001` — un bloc est authentifié avec volume, adresse, format, génération **et magasin**.
  **Ces quatre champs séparent des IDENTITÉS LOGIQUES, et depuis le 11 septembre 2026 les CLÉS sont
  séparées AUSSI** — la revue externe du 10 septembre disait pourquoi la différence compte
  ([#182](https://github.com/pinfada/railsbox-vault/issues/182), HIGH, corrigé le 11 septembre 2026)
  : « les données associées séparent les domaines logiques, mais ne protègent pas AES-GCM contre la
  réutilisation du même couple clé/nonce ». Le format v4 donne à chaque domaine de chaque volume sa
  propre clé dérivée : le RAYON d'une collision de nonce est un (domaine × volume), au lieu de tout
  ce qui vivait sous la DEK. Séparer les clés ne rend pas une collision moins fatale là où elle
  survient ; cela borne ce qu'elle emporte. Le dernier mot du premier paragraphe est une correction,
  pas une précision : jusqu'au format de journal 4, un enregistrement du journal et un secteur du
  volume à la même adresse sous la même génération partageaient leur étiquette de domaine, si bien
  que le sceau de l'un s'ouvrait à la place de l'autre — constat HIGH
  [#143](https://github.com/pinfada/railsbox-vault/issues/143), corrigé par la
  [PR #146](https://github.com/pinfada/railsbox-vault/pull/146), et ce que la correction laisse
  ...
  **depuis #101, l'export passe par un accès BRUT au fichier** (`src/vm/opfs-volume-brut.mjs`), qui
  ...
  RECULE avec un retour arrière du support. **Depuis le format v4, il y a DEUX compteurs et non un**
  — celui du domaine `volume` et celui du domaine `journal` —, tous deux publiés dans l'en-tête
  authentifié de la racine, et chacun compte bien toutes les invocations sous SA clé. Ce que la
  séparation ne corrige pas est écrit avec elle : ils vivent toujours dans la racine, donc ils
  reculent avec elle. Preuves : `tests/unit/vm-format-chiffre-*.test.mjs` — dont
  `vm-format-chiffre-reprise.test.mjs`, qui rejoue la réfutation ci-dessus sur le magasin RÉEL de
  l'ADR 0014 — et les vecteurs figés `tests/vectors/format-chiffre-v1.json`, que #18 devra
...
disparaît avec l'onglet. C'est un endroit de plus où un secret existe — et **cette durée est
désormais BORNÉE**, depuis #169
([ADR 0031](docs/decisions/0031-verrouiller-le-worker-meurt-l-instantane-survit.md)) : deux
déclencheurs la ferment, un geste « Verrouiller » et un **délai d'inactivité de dix minutes**, tenu
entre une minute et une heure par le code et rangé nulle part sur l'appareil. Le verrouillage arrête
la VM, capture l'instantané, ferme les volumes (`close()`), **puis** termine le Worker de confiance,
...
`interne.kek = null` reste une hygiène, pas une promesse. Ce que le verrouillage **ne défend pas**
est écrit au même niveau : la mémoire du processus et le fichier d'échange, un navigateur ou une
extension compromis, l'adversaire qui écrit dans l'OPFS de l'origine de confiance, l'épaule, et la
copie du volume DÉJÀ prise — **verrouiller ne rechiffre pas**, pas plus que révoquer. Voir
l'[ADR 0029](docs/decisions/0029-deverrouillage-dans-la-coquille.md), limite 2 (note datée du 8
septembre 2026).
...
le document applicatif encadré, sur l'autre origine, n'obtient qu'un état et un compte de barrières.
Les réserves qui restent ne portent plus sur la partition, et elles sont nommées dans la colonne.

**#192 ajoute un geste, et pas une réserve** (ADR 0038). Le document encadré est désormais SERVI par
le guest : il demande un chemin HTTP, et la coquille lui rend ce que Rails a répondu. Ce que la
frontière y gagne se dit en une phrase — **le document ne voit toujours que des réponses HTTP** —,
et ce qu'elle n'y perd pas se dit en trois :

- **les dix refus sont INCHANGÉS**, et l'application malveillante les rejoue dans le même relevé que
  les huit tentatives contre le chemin neuf (`tests/browser/coquille-service-applicatif.spec.mjs`,
  trois moteurs) ;
- **le canal privilégié ne gagne aucun type.** Le relais a son propre canal, établi dans la MÊME
  poignée de main, avec son propre vocabulaire ; un type de relais posé sur le port restreint est
  refusé sous `VAULT_COQUILLE_CANAL_DE_RELAIS_REFUSE`, et un type privilégié posé sur le canal de
  relais sous `VAULT_COQUILLE_PORT_PRIVILEGIE_REFUSE` ;
- **le cookie de session Rails ne franchit pas la frontière.** Le bocal vit dans le Worker de
  confiance et meurt avec lui ; `document.cookie` est vide dans la coquille, dans le courtier et
  dans la page servie, et le bocal du contexte du navigateur est vide lui aussi.

**Ce que le routage GARANTIT depuis les revues de la PR #203** : aucune requête n'est servie par le
courtier d'un AUTRE coffre — un courtier est un client qui dit détenir un port restreint, une
sous-ressource suit la liaison de son client, une navigation n'est servie que s'il existe un seul
courtier, et un onglet de premier rang n'est jamais relayé (`src/coquille/routage-du-cadre.mjs`,
mesuré sur deux coquilles réelles par `tests/browser/coquille-deux-coffres.spec.mjs`). Le prix en
est déclaré : un seul coffre servi à la fois.

**Ce que le relais NE garantit PAS, et qu'il faut lire avec le reste** : il ne relaie ni WebSocket
ni `EventSource` ; son bocal ignore l'expiration et les attributs d'un cookie, si bien qu'une
application qui s'appuierait sur l'expiration pour déconnecter ne le serait pas ; il n'a été mesuré
de bout en bout que sous CHROMIUM, Rails n'ayant jamais répondu dans le guest sous Firefox (v86 y
tourne six fois moins vite : un boot Rails y demande environ dix minutes, et le silence du pont
observé à quinze minutes reste inexpliqué) ; et rien n'est su de ce qu'il fait d'une application
Rails autre que celle de référence. **Aucun Service Worker sur l'origine de confiance** : le refus
de l'ADR 0030 décision 4 est inchangé, et celui de la coquille de cadre vit sur l'origine
APPLICATIVE, où l'ADR 0002 l'autorise nommément — il n'ouvre aucune voie vers la coquille que le
port restreint n'ouvre déjà.
...
| `SEC-ORIGIN-001` — coquille de PRODUIT | **exercé**              | la frontière est éprouvée sur les ports RÉELS de la coquille de produit ; le geste de DÉVERROUILLAGE est celui d'un UTILISATEUR depuis #162 ; le CYCLE DE VIE est assemblé depuis #163 — l'ordre des huit étapes est daté, publié et tenu par une garde, et un scénario de bout en bout boote Rails DANS la coquille sur deux origines réelles ; le VERROUILLAGE est livré depuis #169 — un geste (trois moteurs) et un délai d'inactivité de dix minutes (Chromium et Firefox, en temps réel) tuent le Worker de confiance après la fermeture propre, la coquille recharge, un verrouillage REFUSÉ tue et retire le cadre plutôt que de laisser un coffre ouvert, et les dix refus restent identiques — code ET message — sur un coffre ouvert comme sur un coffre verrouillé. le document encadré est SERVI par le guest depuis #192 (ADR 0038) — il demande un chemin HTTP par le port restreint, et ne voit jamais autre chose que des réponses : ni cookie de session (le bocal meurt avec le Worker), ni hôte du guest (une `Location` absolue est réécrite en chemin), ni en-tête non nommé. Ce qui n'est pas couvert n'est pas une réserve sur la partition : le relais n'a été mesuré de bout en bout que sous Chromium (Rails ne répond pas dans le guest sous Firefox : v86 y tourne six fois moins vite, et le silence du pont à quinze minutes reste inexpliqué), il ne relaie ni WebSocket ni `EventSource`, et les fins d'onglet mesurées par moteur sont #170                                                                                                                                                                                                                                                                | `tests/browser/coquille-frontiere.spec.mjs` (application malveillante, dix gestes refusés, témoin positif en même origine, les dix refus identiques dans trois états du coffre, trois moteurs), `tests/browser/coquille-deverrouillage.spec.mjs` (les trois moyens depuis la coquille, la sonde d'exfiltration rejouée APRÈS un verrouillage, trois moteurs), `tests/browser/coquille-cycle-de-vie.spec.mjs` (l'ordre des huit étapes, le refus d'un boot avant le backend, la mort du Worker sur ses trois causes, le verrouillage par geste et par délai d'inactivité réel, le rechargement, trois moteurs), `tests/e2e/reprise-coquille-boot-froid.spec.mjs` (deux origines réelles, boot Rails dans la coquille, verrouillage, instantané constaté sur l'OPFS réel, réouverture par instantané), `tests/browser/coquille-service-applicatif.spec.mjs` (le chemin SERVI : l'épreuve rouge, les six façons d'écrire mal une requête relayée, le canal de relais refusé depuis le port restreint, les dix refus inchangés dans le même relevé, trois moteurs), `tests/e2e/parcours-page-rails.spec.mjs` (une page Rails réelle rendue dans le cadre, cliquée, soumise, relue après un boot à froid ; aucun cookie visible nulle part), `tests/unit/coquille-relais-http.test.mjs`, `tests/unit/coquille-admission.test.mjs`, `tests/unit/coquille-contrat.test.mjs`, `tests/unit/coquille-cycle-de-vie.test.mjs`, `tests/unit/coquille-verrouillage.test.mjs` |
| `SEC-KEY-001`                          | **exercé sous réserve** | RÉSERVE RÉDUITE le 11 septembre 2026 (#182, tranches T2a et T2b) : la DEK est une clé maîtresse importée en matériau HKDF, que WebCrypto refuse de passer à AES-GCM, et les SIX domaines en descendent — `volume`, `journal`, `instantane`, `enveloppe`, `archive`, `recuperation`. Deux volumes sous la même DEK ne partagent aucune clé, les TROIS chemins hors transaction closent par une racine — la racine SUIT le secteur dans la même séquence d'écriture, de sorte qu'une session tuée par `pagehide` n'ait jamais rien de non publié —, le VERSEMENT initial déclare que sa clôture sera la DATATION (invariant tenu par l'appelant, « fait, non garanti »), et le budget est MESURÉ par clé sur une session complète : zéro invocation sous la clé de volume, jamais deux sous une clé à usage unique. Un CLIQUET d'inspection de source le garde, et il mord (dix-huit mutants, dix-huit tués, dont quatre sur ses propres motifs). CE QUI RESTE, et c'est nommé plutôt qu'arrondi : ouvrir un volume **v3** — pour le migrer, ou pour l'exporter avant de le migrer — scelle **3 + N fois** sous la clé de volume elle-même (l'empreinte de région, la racine de clôture, le témoin, plus un par secteur rejoué), et les 3 + N sont comptés dans la racine v3 ; c'est le régime que la v4 remplace, et l'unique exception du cliquet. La RÉSERVE elle-même tient à la clé du chemin de BOOT, qui vient encore du harnais sous jeton. La DURÉE de rétention de la KEK dans le Worker de confiance est BORNÉE depuis #169 (geste « Verrouiller » et délai d'inactivité de dix minutes), et elle est mesurée ; ce que le verrouillage ne couvre pas est écrit avec lui — mémoire du processus, fichier d'échange, copie déjà prise | `tests/unit/vm-budget-par-domaine.test.mjs` (le compte par clé sur une session complète), `tests/unit/vm-cliquet-anti-dek.test.mjs` (le cliquet), `tests/browser/enveloppe-frontiere.spec.mjs`, `tests/browser/coquille-cycle-de-vie.spec.mjs` (le verrouillage par geste et par délai, trois moteurs), `tests/unit/coquille-verrouillage.test.mjs` (l'état, la règle et les bornes du délai)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `SEC-BLOCK-001`                        | **exercé sous réserve** | le format scelle tout, de bout en bout — mais la clé du chemin de BOOT vient encore du harnais, sous jeton. Le constat du 10 septembre 2026 (#182) sur les données associées est CORRIGÉ depuis le 11 septembre : chaque domaine de chaque volume scelle sous sa propre clé dérivée, les six domaines compris. Le budget annoncé est celui que le mécanisme tient, et il est mesuré à l'ÉGALITÉ par le nombre d'invocations réelles ; UNE sous-estimation subsiste, écrite au § 4.5 de la spécification, et elle a une BORNE : une migration reprise peut rescéller une suite deux fois, au plus 512 secteurs par coupure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `tests/unit/vm-volume-chiffre.test.mjs`, `tests/unit/vm-budget-par-domaine.test.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
spécification autonome soumise à la revue externe (#20).
...
lignes, et **les deux constats sont corrigés** : le CRITICAL
[#181](https://github.com/pinfada/railsbox-vault/issues/181) le jour même par la
...
2. **Données sensibles interdites** jusqu'à l'implémentation de la séparation d'origine, du
   ...
   **implémentée depuis #161** — une coquille de produit, un Worker de confiance, un port restreint
   ...
   le reprend sous sa forme humaine ; le **verrouillage est livré depuis #169**
   ([ADR 0031](docs/decisions/0031-verrouiller-le-worker-meurt-l-instantane-survit.md)) — un geste «
   Verrouiller » et un délai d'inactivité de dix minutes tuent le Worker de confiance après la
   fermeture propre des volumes, la coquille se recharge, et un nouveau geste seul rouvre. **Quatre
   des quatre sont donc livrés — et le gate reste FERMÉ.** Ce n'est pas une précaution rhétorique :
   c'est ce que ses propres conditions disent. Les quatre briques sont un COMPTE, pas une
   autorisation — c'était déjà vrai à trois, cela reste vrai à quatre —, et **une démonstration
   réussie ne lève jamais seule un gate**.
...
- **Il est RETIRÉ dès qu'il ne décrit plus l'état présent** : suppression du volume, restauration
  (#12), migration (#13), et toute ouverture qui l'écarte. **Le verrouillage, lui, ne le retire
  PAS** — la position par défaut écrite pour #25 a été rouverte avec un argument et RÉVISÉE le 8
  septembre 2026
