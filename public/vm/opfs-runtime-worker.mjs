...
async function scenarioExclusivite() {
  ...
  // Le registre du module pourrait masquer un support permissif : la demande suivante court-circuite
  // le registre et interroge OPFS directement.
  ...
  const afterCloseSize = third.size();
  await third.close();
...
async function scenarioAdaptateur() {
  ...
  const persistedAfterReopen = (await digestHex(relu)) === writtenDigest;
