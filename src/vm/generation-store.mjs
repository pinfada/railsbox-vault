...
export class GenerationStore {
  ...
  constructor(options) {
    ...
    const { formatVersion } = this.#scellement;
    this.#garde = construireGarde(options);
    this.#sansRacine = construireAutorisation(options);
    // Le format que cette session écrit suit la FRAÎCHEUR qu'elle tient ET la version de format du
    // VOLUME : un numéro de format dit ce que la racine porte, et la v4 lui ajoute un compteur.
    this.#formatEcrit = formatEcritSousFraicheur(this.#garde !== null, formatVersion);
  ...
  async #recupererDepuisRacine({ racine, chargePresente }) {
    ...
    this.#scellement.reprendreDepuisLaRacine(racine);
