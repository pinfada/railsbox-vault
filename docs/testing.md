...
| `npm run test:service-applicatif` | le CHEMIN SERVI (#192) : frontière du relais HTTP, trois moteurs, sans machine virtuelle                                         |                                                                                                                                                                                                                                               environ 80 s |
...
| `npm run test:vm:reference`       | boot à froid réel de l'image de référence sous v86 (disque système COMPOSÉ : rootfs + paquet, ADR 0041)                          |                                                                                                                                                                                                                 plus de 10 min, Docker et artefacts requis |
...
`vault.*` réservé, autorisation de migrer, schéma déduit gardé, forme de quatorze chiffres) ;
**21/21** après la recette QA (« Démarrer » sous la reprise seule ne dit jamais « non servie »). Les
...
partout ; servie par l'origine de confiance, elle obtient le volume, son enveloppe, le DOM, la
fenêtre auxiliaire, la portée d'un Service Worker et l'interception de la ressource témoin. Deux
...
`VAULT_COQUILLE_TYPE_INCONNU` — « je ne sais pas de quoi tu parles » ; après, elle reçoit
`VAULT_COQUILLE_APPLICATION_NON_DEMARREE`. La différence entre ces deux codes EST la tranche.
...
| `tests/unit/coquille-portabilite.test.mjs`           | Node                                      | les DÉCISIONS pures : états de l'emplacement (vide, coffre, antérieur, disque d'un autre coffre, restauration coupée, coffre servi sans manifeste — et une installation coupée qui n'en est pas une), relevé publié avant l'état dit, refus pendant un geste long, en-tête d'archive, cible du coffre, bilan de révocation, dérogation d'archive du contrat                                                                                                                                                                                                                                                                   |
| `tests/unit/coquille-portabilite-du-worker.test.mjs` | Node                                      | le module du Worker TEL QU'IL EST SERVI, sur un magasin en mémoire : sauvegarde au point de contrôle, restauration relue par le code, archive altérée et tronquée refusées sans rien écrire, emplacement occupé, restauration coupée réparée, révocation qui garde l'emplacement ouvrant ; après la revue de la PR #208 : réparation coupée à chacun de ses douze rangs puis reprise, aucune copie de sauvegarde survivante (succès, échec, résidu retiré par la révocation), archive d'un autre coffre refusée avant tout octet, coffre restauré servi puis privé de manifeste jamais réparé, objet déguisé en `File` refusé |
...
| `tests/e2e/durabilite-du-commit.spec.mjs`            | Chromium                                  | la DURABILITÉ DU COMMIT (#209) : une note et sa pièce acquittées, « Verrouiller » à 0, 5 et 30 s (trois fois chacun), instantané retiré, boot à froid — la note et l'empreinte de la pièce sont RELUES, et le disque rouvert accepte l'écriture suivante, relue au boot suivant ; à chaque boot, l'état du disque relevé par l'init (`errors=remount-ro`, 0 erreur au superbloc, 0 alerte `EXT4-fs`) est exigé. Lot 3 de `reprise.yml`                                                                                                                                                                                        |
...
| Suite                                                | Moteurs                                   | Ce qu'elle mesure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/unit/coquille-parcours.test.mjs`              | Node                                      | les ÉCRANS et la PROGRESSION : quel écran pour quel état publié, quelle progression et quel moteur ; `etapeAdmise` qui ramène une étape demandée au-delà de l'étape atteinte, jamais en arrière ; aucun écran de 4 à 9 sans `feuilleEprouvee`, le constat du Worker (#239) — un `parcours.json` falsifié ne choisit que le premier formulaire ; une ouverture de ROUTINE mène à l'étape 4, l'exercice de l'étape 5 à 6, celui de l'étape 8 à 9 ; « Continuer » mène à la prochaine étape non jouée ; « Revenir à mon application » depuis les étapes 5 à 9 et « Parcours terminé » ; l'accueil d'après la visite ; le format 1 de `parcours.json` relu sans preuve ; un code rendu et non éprouvé mène à « Vérifier votre code », et « je n'ai plus cette feuille » ramène à l'annonce, où un SECOND code se crée (#214) ; le COMPTE des feuilles ; la progression écrite ne porte que ses champs ; le masquage de tout code en clair ; les gestes en cours ; les refus du relais annoncés seulement après un démarrage abouti ; Firefox |
| `tests/unit/coquille-ouverture-par-le-code.test.mjs` | Node                                      | OUVRIR PAR UN CODE quand le coffre en porte PLUSIEURS (#214), sur les modules réels et le magasin en mémoire : deux codes ouvrent **dans les deux ordres** ; sept codes, le septième ouvre, et la place sur huit est prise ; un code étranger bien formé est refusé après avoir essayé CHAQUE emplacement ; le coût d'une ouverture ne dépend pas du RANG de la feuille (appels à `SubtleCrypto.decrypt` comptés) ; une ancre trop haute refuse par `VAULT_ENVELOPPE_REJEU`, non par clé refusée ; revue de la PR #219 : le coût d'un `REJEU` ne dépend pas du rang (sept codes, rangs 1 et 7), un type 4 aux paramètres illisibles ne masque pas le code valable posé après lui, un code mal recopié coûte une lecture et zéro déchiffrement. Ce qu'elle ne mesure pas : l'OPFS, le port privilégié, le temps d'HORLOGE d'un refus, et l'API générique `ouvrirVolumeParDerivateur` : son refus des emplacements ambigus (#220), avant dérivation et sans écriture, est éprouvé séparément dans `vm-derivation-branchement.test.mjs`     |
| `tests/unit/coquille-parcours-conduites.test.mjs`    | Node                                      | le CLIQUET DES CONDUITES, PAR CONSTRUCTION : chaque code des sept familles exportées (coquille, stockage, enveloppe, dérivation, archive, import, et la politique de phrase depuis #240) est sur le chemin avec sa conduite et son classement, ou écarté par un motif « inatteignable depuis le parcours parce que … » ; les refus sans code traduits ; tout texte montré (écrans, messages, conduites) sans vocabulaire interne, et le filtre mord sur « secteur » et « génération »                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `tests/unit/coquille-parcours-relecture.test.mjs`    | Node                                      | la PAGE DE RELECTURE est à jour (`node tools/relecture-parcours.mjs` ne change rien) et reproduit chaque texte servi, y compris ceux écrits sous le champ de la phrase (règle des 12 caractères, #242) ; la limite de Firefox est écrite de la même phrase dans l'ADR 0040, le README, `docs/compatibility.md` et ce document                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `tests/unit/coquille-preuve-de-la-feuille.test.mjs`  | Node                                      | la PREUVE DE LA FEUILLE (#239) : l'enregistrement du secteur 1 (marque, format 1, huit identifiants au plus) ; un secteur vierge, une marque étrangère, un format à venir, un compte hors borne se lisent « aucune feuille » ; sur un VRAI volume `coquille` scellé, un secteur jamais écrit rend 512 zéros authentifiés, et l'inscription survit à la fermeture ; sur une vraie enveloppe, le neuvième emplacement est refusé par `VAULT_ENVELOPPE_PLEINE` sans rien écrire ; dans la source du Worker, la preuve est constatée APRÈS l'ouverture du volume et AVANT sa barrière, et seul un code l'inscrit                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `tests/browser/coquille-parcours.spec.mjs`           | Chromium, Firefox, WebKit (limite exigée) | l'ORDRE ATTAQUÉ sur la coquille réelle, sans machine virtuelle : un rechargement après l'affichage du code ne produit aucun second `creer-recuperation` (compte = 1) et le code rendu rouvre le coffre ; `?etape=4`, `6`, `9` ramènent à l'étape 3 ; aucun lien vers la vue complète ; la feuille s'éprouve en verrouillant puis en rouvrant par le code, sans code en clair dans `document.body.innerHTML` ni dans `parcours.json` ; ensuite la phrase rouvre, l'étape 5 se joue par la phrase, « Revenir à mon application » et une ouverture de routine mènent à l'étape 4 ; un coffre d'avant #239 demande son code une fois, puis plus ; un `parcours.json` falsifié n'ouvre aucun écran de travail ; une enveloppe pleine dit sa sortie et la révocation fait de la place ; une phrase trop courte reçoit sa conduite (#240) ; « J'ai oublié ma phrase » ramène à l'étape 4 sans rien marquer de la visite ; Firefox dit sa limite à l'étape 4. 27 épreuves, 1,5 min en local sur les trois moteurs (19/09/2026)                   |
| `tests/e2e/parcours-utilisateur.spec.mjs`            | Chromium                                  | le PARCOURS UTILISATEUR : les neuf étapes sur la coquille réelle et Rails réel (A crée, éprouve sa feuille en verrouillant puis en rouvrant par le code, travaille, verrouille, rouvre par la PHRASE, sauvegarde, revient à l'application qui redémarre et relit la note ; B restaure, récupère par le code, révoque, revient à l'application qui démarre à froid et relit la même note), en ne touchant la page QUE par ce qu'une personne voit ; les trois échecs les plus probables ; aucune alerte et le bouton « Démarrer » fermé pendant le boot ; Entrée qui valide ; aucun code en clair. Délai du scénario : 15 min. Lot 3 de `reprise.yml`                                                                                                                                                                                                                                                                                                                                                                                     |
| `tools/muter-gardes-coquille.mjs`                    | Node                                      | vingt-trois mutants sur les gardes du parcours — les quatre de la première livraison, quatorze de la revue (progression, étape admise, confirmation exigée, vérification du code, masquage, gestes en cours, relais, Firefox, cliquet par construction, refus sans code), trois de #214 (le `find` qui CHOISISSAIT un emplacement de type 4, le court-circuit après un succès, le compte des feuilles replié sur un booléen), et deux de la revue de la PR #219 (un refus levé dans la boucle d'ouverture par code, la dérivation sortie du `try`) —, tous tués : 111/111 pour la campagne, 302/302 pour les onze (16/09/2026)                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Suite                                      | Moteurs                   | Ce qu'elle mesure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------ | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `tests/apparence/parcours.spec.mjs`        | Chromium, Firefox, WebKit | l'ACCESSIBILITÉ et le RENDU des premiers écrans, par exécution : axe (règles A et AA jusqu'à WCAG 2.2), en clair et en sombre, sur Créer, Choisir et Restaurer une sauvegarde aux largeurs 320, 768, 1024 et 1440, sur la feuille aux quatre largeurs, sur l'annonce du code, la confirmation, le refus de recopie et l'étape 4 avant démarrage à une largeur ; un écran de plus par moteur à 320 (Firefox : l'étape 4 et sa limite ; WebKit : le refus du moteur) ; aucun groupe sans nom dans la région de l'étape ; les contrastes recalculés depuis les styles calculés (texte ≥ 4,5:1, bordure de champ ≥ 3:1) ; aucun débordement horizontal ; la feuille de récupération à l'impression, puis son absence après confirmation ; le focus clavier visible (contour ≥ 3 px) ; WebKit qui refuse le stockage le dit                                                                                                                                       |
| `tests/apparence/parcours-etapes.spec.mjs` | Chromium                  | les ÉTAPES 4 PRÊTE À 9 : l'étape 4 prête en **état simulé** (la ligne d'état du cycle posée par l'épreuve, sans machine virtuelle) aux quatre largeurs, puis Verrouiller, Rouvrir, Sauvegarder et son refus (un coffre jamais démarré n'a rien à sauvegarder), Restaurer sur un autre appareil, Récupérer, Révoquer, Parcours terminé JOUÉS sur la coquille réelle, à 320 et 1024, en clair et en sombre ; « Vérifier votre code » après un rechargement ; `forced-colors: active` (bordures et contour de focus, création, choix, étape 4 prête simulée) ; mouvement non réduit (barre indéterminée du démarrage, état simulé) ; messages de conduite à 16 px, contour de « J'ai oublié ma phrase » ≥ 3:1. Les deux fichiers : 38 scénarios, `npm run test:apparence` (dans `npm run check`), 100 s en local (15/09/2026 ; 77 s avant la revue) ; captures et rapports axe dans `reports/apparence/`, versés par la CI dans l'artefact `apparence-parcours` |
...
Chaque contexte que ces deux fichiers créent — la page du scénario, l'appareil vierge de la
restauration — est surveillé du premier au dernier geste : aucune requête hors des deux origines
servies (écouteur `request` du contexte), et aucune violation de CSP dans aucun de ses documents,
cadres compris (écouteur `securitypolicyviolation` posé par `addInitScript`, remonté par une liaison
qui survit aux rechargements). Une requête refusée par la CSP n'arrive pas au réseau : c'est le
second écouteur qui la voit. Seule exception, nommée : la feuille `<style>` que WebKit insère pour
prendre une capture, comptée à part pendant la capture et seulement elle. Témoins négatifs joués en
local le 15/09/2026 : une feuille injectée et une requête vers une troisième origine font échouer le
scénario.
...
violation automatique n'est pas une certification ; l'étape 4 prête autrement que par simulation
(c'est l'E2E clavier qui la prouve, sans capture) ; « Sauvegarde prête », la restauration réelle
d'une archive et l'écran qui la suit, qui exigent un premier démarrage ; les étapes 5 à 9 sous
Firefox et WebKit ; les écrans de refus d'inventaire. Le témoin de fond clair de
`tests/e2e/parcours-page-rails.spec.mjs` ne vaut qu'en thème clair : sous
`prefers-color-scheme: dark`, `vault.css` remplace le fond de la page Rails.
...
| unitaire   | `tests/unit/coquille-ouverture-par-le-code.test.mjs` | N feuilles ouvrent le coffre, sans que le coût désigne laquelle (#214)                                                                                                                                                                                                                                                                                                                                                                                    | `npm run check` |
...
| navigateur | `tests/browser/deverrouillage-frontiere.spec.mjs`    | un SECOND code ouvre le volume sur l'OPFS réel, et le premier aussi (#214) ; durées d'ouverture entière publiées — Chromium 8 ms par le second et 6 ms par le premier, Firefox 57 ms et 48 ms (16/09/2026), WebKit sans OPFS synchrone rend son refus typé. Aucun budget ne leur est opposé : un échantillon par exécution ne mesure pas une distribution, et ce qui prouve l'indépendance au rang est le nombre d'appels AEAD, compté au niveau unitaire | `npm run check` |
...
avant l'en-tête v4. La campagne de #181 (`muter-gardes-archive-recuperation.mjs`, 30/30 avec #220)
reste verte.
...
Worker avec son handle exclusif — sans `close()`, sans barrière. Ces compteurs sont cumulés : ils ne
garantissent PAS qu'une écriture non validée suit la dernière barrière (#222).
...
2. **la récupération DIT ce qu'elle a fait** : dans ce scénario une génération validée doit être
   `rejouee`, avec un nombre d'enregistrements strictement positif. Avant cette récupération,
   `inspect-generation` lit seulement les racines et la taille du journal survivant, sans créer,
   écrire, flusher ni rejouer. Le test compare EXACTEMENT la génération, les enregistrements rejoués
   et les octets écartés à ce constat. Zéro octet écarté n'est accepté que si le journal observé
   n'avait aucune queue non validée ;
...
Le constat structurel n'est pas une authentification : le boot suivant vérifie les sceaux. Les
épreuves de `vm-generation-store.test.mjs` imposent les deux cas (queue nulle et non nulle),
confrontent le constat au vrai récupérateur, et refusent un journal absent ou tronqué.
`journal-apres-coupure.json` et `generation-mesuree.json` publient la mesure brute (#222).
...
**Un TROISIÈME lot, dédié à la durabilité du commit (#209, revue de la PR #211, constat 1).**
`durabilite-du-commit.spec.mjs` a d'abord rejoint le lot 2 ; la recette du commit e3f63ef
(run 34775718482) a mesuré ce lot à **58 min 23 s pour une borne de 60**, dont 24,2 min pour ce seul
scénario — dix boots de Rails, que la dispersion des exécutants (71 à 118 s par boot selon les
recettes récentes) aurait suffi à faire dépasser. Il a donc son lot, le 3, et sa répétition ×3 par
délai est CONSERVÉE en CI : la preuve du gate de persistance s'y joue entière, chaque nuit. Durées
CI mesurées ou déduites de ce run, un ouvrier par lot : **lot 1 36 min**, **lot 2 ≈ 34 min** (58 min
23 s moins les 24,2 min du scénario), **lot 3 ≈ 26 min** (24,2 min de scénario et la préparation
...
| 3   | 17 min 28 | 12 min 12        | 2 sur 328         | **rouge** — `fins-d-onglet.spec.mjs:681`, `page.goto` expiré |
| 4   | 20 min 20 | 14 min 00        | 6 sur 328         | **rouge** — `fins-d-onglet.spec.mjs:431`, `page.goto` expiré |

(Le run 4 porte le commit final ; il ne diffère du commit des trois premiers que par cette page.)

Les rouges méritent d'être lus en entier, parce qu'ils sont le fait le plus dur de cette tranche.
Ils ne viennent ni d'une régression, ni d'une épreuve du produit : `test:fins-d-onglet` joue avec
`retries: 0`, et le flottement de #178 l'y a saisi **deux fois sur cinq runs de cette branche** —
`fins-d-onglet.spec.mjs:681` puis `:431`, chaque fois `page.goto: Timeout 60000ms exceeded`, le
geste et le message exacts des trente autres pertes relevées ici. **Le flottement peut donc rendre
le gate ROUGE**, et pas seulement lui manger sa marge : il suffit qu'il tombe sur la seule suite qui
