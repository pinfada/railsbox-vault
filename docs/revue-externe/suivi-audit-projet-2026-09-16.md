...
| 2. Surveillance des dépendances                   | Implémentée et constatée sur GitHub                        | Dependabot couvre npm, Bundler et Actions. `security.yml` définit deux audits indépendants, appelés par la CI et la publication, également exécutables sur calendrier. Le contrôle déjà requis `Qualité et tests` refuse désormais un échec des audits ou de la couverture. Les deux chemins sont constatés sur GitHub : positif (PR #224, run 35199493648) et négatif (PR #229, jetable, fermée sans fusion, run 35228193355), voir la section ci-dessous.                           |
...
**Ce mécanisme est constaté le 17/09/2026, sur les deux chemins.** Chemin POSITIF : PR #224 (tranche
de la chaîne d'approvisionnement), run 35199493648, tête `5447e7f8` — tous les jobs en `success`,
dont `Qualité et tests` (job 105136239229). Chemin NÉGATIF : une PR jetable
(`jetable/gate-negatif-224`, PR #229), qui n'a fait tomber QUE la couverture unitaire (un module
chargé sans être appelé, sous les trois planchers), sans toucher aux vérifications du produit ni aux
audits ; run 35228193355. Jobs et conclusions : `Vérifications du produit` `success` (105225064005),
`Couverture unitaire (Node 22)` `failure` (105225064213), `Couverture unitaire (Node 24)` `failure`
(105225064205), `Audit npm et inventaire du verrou` `success` (105225064436), `Audit du verrou Ruby`
`success` (105225064975), `Campagnes de mutation` `success` (105225063702), et
**`Qualité et tests`** `failure` (105232304456) — jamais `skipped` ni `cancelled`. La PR #229 a été
fermée sans fusion (`gh pr close --delete-branch`) une fois le constat relevé ; sa branche distante
n'existe plus.
