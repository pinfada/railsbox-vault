...
**Où en est cet assemblage, au 8 septembre 2026.** Il l'est, et il faut le dire étape par étape
...
| 7     | **PRODUIT** — le chemin depuis #163 (arrêt de la VM, instantané, `close()`, puis `terminate()`, dans cet ordre) ; le DÉCLENCHEUR, le DÉLAI et le sens de « verrouillé » depuis #169 : le geste s'appelle « Verrouiller », un délai d'inactivité de dix minutes l'atteint seul, et la coquille RECHARGE pour retirer le cadre ([ADR 0031](decisions/0031-verrouiller-le-worker-meurt-l-instantane-survit.md)). Depuis #170, les FINS D'ONGLET sont mesurées par moteur et la coquille en fait quelque chose : `pagehide` termine le Worker tout de suite, un retour depuis le bfcache recharge, le gel ne tue rien, un retour vérifie l'échéance, et `beforeunload` n'est jamais branché ([ADR 0032](decisions/0032-les-fins-d-onglet-ce-que-le-moteur-livre.md)) |
...
VERROUILLAGE, rechargement, instantané constaté sur l'OPFS réel et réouverture par ce même
instantané — et ceux qui partent de `/vm/reference.html` restent : le banc demeure le témoin de la
reprise hors ligne.
