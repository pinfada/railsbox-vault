...
| Base                 | SQLite, `journal_mode = delete`, `synchronous = extra` (ADR 0004, #209)      |
...
| Écriture acquittée                     | RPO 0 **dès l'acquittement**, inconditionnel pour les écritures SQLite et les pièces jointes ActiveStorage de l'application de référence (#209) : commit `synchronous = extra`, pièce `DurableDisk`, disque ext4 journalisé — mesuré 9/9 relues après verrouillage à 0, 5 et 30 s puis boot à froid, le disque rouvert acceptant chaque fois l'écriture suivante (13/09/2026, Chromium, local ; avant : 4/4 perdues à 0 s). Non couvertes : les écritures d'une application tierce hors SQLite et ActiveStorage (#210)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Portabilité (`VAULT-PORT-001`)         | un volume exporté est restauré et OUVRABLE sur un autre appareil — **prouvé pour v2 depuis #12**, et **prouvé pour v3 avec un moyen de récupération depuis #149 (ADR 0027)** : le cycle export → restauration → ouverture PAR LE CODE → lecture du clair est prouvé au niveau unitaire (`tests/unit/vm-restauration-recuperation.test.mjs`), et le bout en bout inter-origine avec boot Rails (`tests/e2e/archive-recuperation-inter-origine.spec.mjs`) est **vert en CI** — run `Reprise MVP` 34050810833 du 6 septembre 2026, 56 min. **Ce qui reste hors gate est dit** : un volume v3 SANS moyen de récupération produit toujours une archive que personne n'ouvre ailleurs, et le produit ne l'empêche pas — il le déclare (`recovery: null`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Déverrouillage depuis la coquille      | l'attente est ANNONCÉE avant d'être subie, et les deux délais sont publiés depuis le GESTE — **mesuré depuis #162 (ADR 0029)** : geste → annonce sous 20 ms sur les trois moteurs, geste → coffre ouvert 400 à 700 ms (Chromium) et 2 100 à 2 600 ms (Firefox), pour une attente annoncée de 446 et 2 207 ms lue de la table de l'ADR 0021. **Ce qui est AFFIRMÉ par l'épreuve** est le seul rapport qui ne dépende pas de la machine : l'annonce précède l'ouverture. Aucun seuil n'est posé — un seuil sur un exécutant partagé mesurerait la charge de l'exécutant. Sur WebKit, l'état rendu est `indisponible` : l'OPFS synchrone manque au Worker, et ce n'est pas le geste qui a échoué                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Famine du Worker de confiance          | le geste ADMIS du document applicatif est servi PENDANT une dérivation — **mesuré depuis #162 (ADR 0029, décision 5)** : question d'état au repos p50 0,2 ms / max 0,9 ms (Chromium) et 1 / 2 ms (Firefox) ; **pendant** une dérivation Argon2id, 0,2 / 0,9 ms et 1 / 2 ms. Avant la correction, la revue de la PR #167 mesurait **1 777 à 2 158 ms sous Firefox** : Argon2id est un appel WebAssembly SYNCHRONE, et le fil du Worker de confiance ne dispatchait plus aucun message. La dérivation vit désormais dans un Worker DÉDIÉ. **Ce qui est AFFIRMÉ** est le seul rapport indépendant de la machine : le maximum observé reste sous le budget que le document applicatif s'accorde (1 000 ms, relu de la fixture)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Coquille de produit (`SEC-ORIGIN-001`) | l'assemblage de la frontière publie ce qu'il coûte, sans seuil — **mesuré depuis #161** : canal privilégié établi en 100 ms (Chromium), 345 ms (Firefox), 560 ms (WebKit) en médiane sur cinq exécutions ; cadre applicatif chargé en 318 ms, 3 879 ms et 761 ms. **Ce qui reste hors gate est dit** : le cycle de vie est ASSEMBLÉ depuis #163 (ADR 0030) — l'ordre des huit étapes est daté et publié étape par étape (`#coquille-rapport` › `cycle`), et `reports/e2e/reprise-coquille.json` publie le prix d'un boot Rails DANS la coquille à côté de celui du banc ; le VERROUILLAGE est mesuré depuis #169 (ADR 0031) — geste → coffre `verrouille` publié sans seuil dans `mesures.verrouillageMs` sur les trois moteurs, délai d'inactivité de dix minutes éprouvé pour de bon à sa borne d'une minute, sur Chromium et Firefox (WebKit n'ouvre rien, donc n'arme rien), et prix de la réouverture publié dans `reports/e2e/reprise-coquille.json` — l'instantané SURVIT au verrouillage, et le relevé du 8 septembre 2026 le chiffre : verrouiller 1 859,5 ms, rouvrir par instantané 1 038,2 ms, contre 114,1 s de boot à froid, soit un rapport de cent dix pour un ; les FINS D'ONGLET sont mesurées par moteur depuis #170 (ADR 0032) — `pagehide` livré à la fermeture sur les trois moteurs, un retour arrière depuis le bfcache qui RESTAURE la coquille sur Chromium fenêtré et que `pageshow` fait recharger jusqu'à `verrouille` en deux chargements, et un gel que le harnais ne sait pas provoquer, chaque ligne « non simulable » avec son motif dans `docs/compatibility.md` ; le document encadré est SERVI par le guest depuis #192 (ADR 0038, mesures au § « Ce que SERVIR l'application coûte ») ; l'ouverture par un SECOND code de récupération est échantillonnée depuis #214 (PR #219, en recette) — ouverture entière 8 ms par le second et 6 ms par le premier sous Chromium, 57 et 48 ms sous Firefox, le 16/09/2026, sans budget opposé ; ce qui reste hors de cette mesure est le SIGNAL DE PRÉSENCE du cadre (#176), sans lequel une personne qui travaille dans l'application est comptée comme absente |
...
[`spikes/0185-aes-gcm-siv.md`](spikes/0185-aes-gcm-siv.md).
...
le guest réel de l'image #5. Il n'est joué par AUCUNE recette — `vm.yml` ne construit pas l'image de
référence, et la suite s'y ignore (revue d'intégration de la PR #203, constat 11) : c'est une mesure
LOCALE, et son relevé brut est donc versionné.
...
**Une page complète coûte environ 540 à 750 ms** — document plus trois actifs —, soit sept à dix
fois moins que le seuil de cinq secondes que l'issue #192 posait comme condition d'abandon.
...
### Le prix d'une écriture durable dès l'acquittement (#209)
...
| bornes d'une requête relayée               | pont série 120 s < coquille 150 s < Service Worker 180 s : la plus EXTÉRIEURE attend le plus longtemps, et le refus le plus précis arrive le premier (l'ordre est exigé par une épreuve) |
| en-têtes sur ce que le Service Worker sert | ceux de l'hébergeur pour le rôle `app` — `nosniff`, CORP, `Cache-Control: no-store` —, rejoués (revue #203, 7)                                                                           |
...
relais, le pont, Rails, et le RENDU. Banc : `tests/e2e/parcours-page-rails.spec.mjs`, Chromium,
**deux exécutions consécutives le 13 septembre 2026**, après les corrections des revues de la PR
#203 — c'est la table qui fait foi ; celles du 12 septembre (trois exécutions ici, quatre dans le
corps de la PR) décrivaient un produit qui déclarait le Worker mort par silence sur d'autres
machines.

| Geste                                                            | 1        | 2        |
| ---------------------------------------------------------------- | -------- | -------- |
| boot de l'application (premier démarrage, installation comprise) | 116,6 s  | 82,0 s   |
| **plus long silence du Worker pendant ce démarrage**             | 8,3 s    | 5,7 s    |
| **première page servie**, après le démarrage                     | 157 ms   | 266 ms   |
| page suivante (un clic dans la page réelle)                      | 310 ms   | 337 ms   |
| soumission du formulaire, 303 suivie, page rendue                | 1 515 ms | 953 ms   |
| réouverture après verrouillage (par l'instantané)                | 1 281 ms | 1 084 ms |
| première page servie après le boot à froid                       | 28 ms    | 34 ms    |
| durée du scénario entier                                         | 3,6 min  | 2,9 min  |

Le même jour, le témoin `tests/e2e/reprise-coquille-boot-froid.spec.mjs` — qui porte désormais la
coquille de cadre ACTIVE — passe deux fois (2,7 et 2,8 min), sans neutraliser le Service Worker.

**Le plus long silence du Worker est la grandeur de la correction I1.** La borne de mort par silence
est de trente secondes (ADR 0030, décision 3), et le scénario exige que ce silence reste sous la
moitié. Avant la correction, mesuré dans Chrome sur le même premier démarrage : 22,6 s coquille de
cadre active, 32,2 s coquille de cadre neutralisée, 84 s onglet caché — la cause était
l'installation du disque applicatif, dont les boucles ne rendaient jamais la main (ADR 0038,
décision 5 bis), et non le relais.

**La première page tombe en quelques dizaines à quelques centaines de millisecondes** : le cadre
attend l'ANNONCE DE BARRIÈRE qui suit le démarrage, puis redemande sa page une fois.

**La réouverture coûte 1,1 à 1,3 s au lieu de 80 à 120 s** : c'est l'instantané que le verrouillage
laisse (ADR 0031, décision 3), mesuré ici sur un parcours d'utilisateur plutôt que sur un banc.

**La soumission, mesurée dans le navigateur (0,95 à 1,52 s), coûte plus que la somme des deux
requêtes du pont.** L'écart est le RENDU et les deux navigations que la redirection impose au moteur
— la page d'arrivée est chargée en entier, avec ses trois actifs. C'est pour cela que les deux
tables sont publiées côte à côte : l'une dit ce que le pont coûte, l'autre ce que le parcours coûte.
...
le guest tourne et Python vit — puis se tait ; Puma n'écoute jamais. Firefox paie six fois le prix
de Chromium sur v86 (#74, reconfirmé le 12 septembre 2026 : invite du guest à 3 852 ms sous
...
Aucun budget n'a été relâché pour faire passer ce moteur. L'épreuve s'ignore en NOMMANT sa cause
plutôt que de passer au vert par vacuité, et `compatibility.md` porte la même phrase.
