...
| Portabilité (`VAULT-PORT-001`) | un volume exporté est restauré et OUVRABLE sur un autre appareil — **prouvé pour v2 depuis #12**. Pour **v3 avec un moyen de récupération (#149, ADR 0027)** : le cycle export → restauration → ouverture PAR LE CODE → lecture du clair est prouvé au niveau unitaire (`tests/unit/vm-restauration-recuperation.test.mjs`) ; le bout en bout inter-origine avec boot Rails (`tests/e2e/archive-recuperation-inter-origine.spec.mjs`) est **À PROUVER EN CI** — run `Reprise MVP` 34048456240, non encore vert au moment d'écrire. La phrase passera au présent quand un run le sera, et pas avant. **Ce qui reste hors gate est dit** : un volume v3 SANS moyen de récupération produit toujours une archive que personne n'ouvre ailleurs, et le produit ne l'empêche pas — il le déclare (`recovery: null`) |
