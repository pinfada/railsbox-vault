...
| Portabilité (`VAULT-PORT-001`) | un volume exporté est restauré et OUVRABLE sur un autre appareil — **prouvé pour v2 depuis #12** ; pour **v3 avec un moyen de récupération depuis #149** (ADR 0027) : le cycle export → restauration → ouverture PAR LE CODE → lecture du clair est mesuré au niveau unitaire (`tests/unit/vm-restauration-recuperation.test.mjs`), et le bout en bout inter-origine avec boot Rails est `tests/e2e/archive-recuperation-inter-origine.spec.mjs`, joué par la recette `Reprise MVP`. **Ce qui reste hors gate est dit** : un volume v3 SANS moyen de récupération produit toujours une archive que personne n'ouvre ailleurs, et le produit ne l'empêche pas — il le déclare (`recovery: null`) |
