...
| `VAULT_COQUILLE_INSTALLATION_INACHEVEE`           | le PREMIER boot d'une application installée n'a pas pu acquérir ses artefacts — noyau, initrd, rootfs, paquet : refusés, coupés ou d'une empreinte fausse — et le volume n'a jamais démarré (sa seule racine est celle de la naissance, #250). Rien n'est perdu ; la page offre « Reprendre l'installation », qui redémarre sans rien retirer : le volume est identifié. La page lit AUSSI une installation reconnue sous `VAULT_COQUILLE_VOLUME_APPLICATIF_SANS_MANIFESTE` quand la réponse porte `installationInterrompue: true` (ADR 0037, note du 19/09/2026)                                                               |
| `VAULT_COQUILLE_ARTEFACT_DU_DEMARRAGE_REFUSE`     | un artefact du DÉMARRAGE — noyau, initrd, rootfs, paquet — n'a pas pu être acquis (refusé, coupé, d'une empreinte fausse) sur un volume qui a **déjà démarré** (#255). Le boot acquiert AVANT d'ouvrir le volume : rien n'a été ouvert ni modifié, et les données du coffre sont intactes. La page dit que cette adresse n'a pas fourni l'application, offre « Sauvegarder mon coffre » et « Verrouiller mon coffre », et **jamais** « Reprendre l'installation » — une reprise écraserait ce que le guest a écrit. C'est le symétrique de `VAULT_COQUILLE_INSTALLATION_INACHEVEE`, dont il ne diffère que par l'état du volume |
...
**SAUVEGARDER, RESTAURER, RÉVOQUER depuis la coquille** (#207,
[ADR 0039](decisions/0039-sauvegarder-restaurer-revoquer-depuis-la-coquille.md)). Trois types NEUFS
du canal privilégié, arrivés par le chemin de l'ADR 0028 : un ADR, cette table, et le cliquet
`tests/unit/coquille-types-documentes.test.mjs`. Aucun ne parvient au document applicatif, qui
reçoit `VAULT_COQUILLE_PORT_PRIVILEGIE_REFUSE` sur le type seul. Une archive altérée ou tronquée
garde le code de sa famille (`VAULT_ARCHIVE_DIGEST_MISMATCH`, `VAULT_ARCHIVE_TRUNCATED`, § 10) : ce
sont deux remèdes distincts.

| Code                                         | Ce qu'il constate                                                                                                                                                                                                                                                                             | Conduite                                                                                                                  |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `VAULT_COQUILLE_COFFRE_ANTERIEUR`            | le coffre de cet appareil a été créé avant le 13/09/2026 : son volume `coquille` porte encore l'identité du coffre. Une seule cause (revue de la PR #208, constat 5). Rendu dès l'INVENTAIRE, avant toute dérivation                                                                          | effacer les données du site et recréer le coffre ; aucune migration, rien n'est publié                                    |
| `VAULT_COQUILLE_DISQUE_D_UN_AUTRE_COFFRE`    | le manifeste du disque applicatif ne déclare pas le volume du coffre (un autre identifiant, ou un format que la coquille n'écrit pas) : ni installé ni restauré par elle. Rendu dès l'INVENTAIRE                                                                                              | effacer les données du site, puis restaurer une sauvegarde du coffre ; rien n'est ouvert ni réparé                        |
| `VAULT_COQUILLE_ARCHIVE_D_UN_AUTRE_COFFRE`   | l'archive présentée à la restauration ne déclare pas le volume d'un coffre de la coquille. Refusé AVANT tout octet écrit : l'emplacement reste tel qu'il était                                                                                                                                | choisir le fichier produit par « Sauvegarder le coffre »                                                                  |
| `VAULT_COQUILLE_EMPLACEMENT_OCCUPE`          | une restauration sur un emplacement qui porte déjà un coffre, ouvert ou non                                                                                                                                                                                                                   | restaurer sur un emplacement vide : on ne restaure jamais par-dessus                                                      |
| `VAULT_COQUILLE_RESTAURATION_INTERROMPUE`    | une restauration coupée avant son manifeste : un disque sans enveloppe, ou à côté d'une page du domaine `recuperation` jamais mutée — et aucune trace de service. Rendu par l'inventaire et le déverrouillage                                                                                 | relancer la restauration avec la même archive : le même geste répare                                                      |
| `VAULT_COQUILLE_COFFRE_SERVI_SANS_MANIFESTE` | la signature d'une restauration coupée, plus une trace de service (volume `coquille`, journal de génération, témoin ou instantané du disque) : un coffre restauré qui a SERVI, et dont le manifeste s'est perdu. Rendu par l'inventaire, le déverrouillage et la restauration ; jamais réparé | ce coffre a servi, sa réparation demande une décision : n'effacer ni les données du site ni l'archive, demander de l'aide |
| `VAULT_COQUILLE_ARCHIVE_SANS_RECUPERATION`   | l'archive déclare `recovery: null` : le coffre restauré ne s'ouvrirait par rien. Refusé AVANT toute écriture                                                                                                                                                                                  | restaurer une archive prise après la création d'un moyen de récupération                                                  |
| `VAULT_COQUILLE_APPLICATION_NON_INSTALLEE`   | une sauvegarde demandée sur un coffre sans application installée : il n'y a rien à sauvegarder                                                                                                                                                                                                | démarrer l'application une première fois                                                                                  |
| `VAULT_COQUILLE_GESTE_EN_COURS`              | un geste de portabilité arrivé pendant un geste LONG (démarrage, reprise, sauvegarde, restauration). Refusé à l'arrivée, jamais mis en file                                                                                                                                                   | réessayer quand le geste en cours a abouti                                                                                |
| `VAULT_COQUILLE_APPLICATION_ETRANGERE`       | au démarrage (après le déverrouillage, avant tout boot) : le manifeste du volume `application` déclare un autre `app.id` que le descripteur servi (#236 T2, ADR 0042). Rien n'est booté ni écrit                                                                                              | garder le coffre ; le rouvrir sur une origine qui sert son application ; une sauvegarde se fait sans boot                 |
| `VAULT_COQUILLE_APPLICATION_NON_SERVIE`      | au démarrage : l'origine ne sert aucune application, ou ne sert plus la version du coffre sans le geste de mise à jour (rétention 1 épuisée). Les données sont intactes                                                                                                                       | garder, sauvegarder, rouvrir sur une origine qui la sert — ou « Mettre à jour l'application » s'il est proposé            |
| `VAULT_COQUILLE_APPLICATION_ANTERIEURE`      | au démarrage : le schéma du paquet servi est INFÉRIEUR à celui des données (retour arrière, origine en retard). Ni boot, ni migration                                                                                                                                                         | garder, sauvegarder, rouvrir plus tard ou ailleurs                                                                        |
| `VAULT_COQUILLE_SCHEMA_DU_COFFRE_INCONNU`    | au démarrage : un manifeste d'avant T2 sans `app.schema`, et aucun paquet servi de la même `app.version` pour en dire le schéma                                                                                                                                                               | rouvrir sur une origine qui sert cette version ; le schéma n'est pas deviné                                               |
| `VAULT_COQUILLE_SCHEMA_DIVERGENT`            | constaté par le GUEST avant Rails : le marqueur `/app/var/.vault-schema` ne dit ni le schéma du manifeste ni celui du paquet, ou il dépasse celui du paquet. Rails n'est pas lancé                                                                                                            | « Mettre à jour l'application » reprend une mise à jour interrompue ; sinon restaurer la sauvegarde faite avant           |
| `VAULT_COQUILLE_MIGRATION_ECHOUEE`           | `db:migrate` a échoué dans le guest : Rails n'est pas lancé, le marqueur n'a pas bougé, le manifeste non plus                                                                                                                                                                                 | restaurer la sauvegarde faite avant la mise à jour ; signaler l'échec à l'auteur du paquet                                |
| `VAULT_COQUILLE_MISE_A_JOUR_INTERROMPUE`     | au démarrage : le manifeste porte l'intention d'une mise à jour commencée, et l'origine ne sert pas une version STRICTEMENT plus récente que celle du coffre, d'un schéma au moins égal à la cible (revue de sécurité de la PR #249, constat 1). Ni boot ni écriture                          | rouvrir plus tard ou sur une origine qui sert la version visée ; une sauvegarde se fait sans boot                         |
| `VAULT_COQUILLE_MISE_A_JOUR_A_REPRENDRE`     | au démarrage SANS le geste : le manifeste porte l'intention d'une mise à jour commencée, et l'origine sert de quoi la reprendre (recette QA de la PR #249, Q1). Jamais « non servie ». Ni boot ni écriture                                                                                    | « Reprendre la mise à jour » ; une sauvegarde se fait sans boot                                                           |
| `VAULT_COQUILLE_MARQUEUR_DE_SCHEMA_INVALIDE` | constaté par le GUEST avant Rails : un marqueur de schéma (données, paquet, intention) ou le schéma attendu n'est pas un entier de quatorze chiffres au plus, sans zéro de tête. Rien n'est migré ni réécrit                                                                                  | garder le coffre, le sauvegarder, demander de l'aide                                                                      |
| `VAULT_COQUILLE_PARAMETRE_DU_GUEST_REFUSE`   | constaté par le GUEST : un paramètre `vault.*` apparaît deux fois sur la ligne du noyau. L'espace est réservé au Worker de confiance, et le descripteur servi le refuse déjà                                                                                                                  | rouvrir depuis une autre origine ; signaler la publication                                                                |
| `VAULT_COQUILLE_MIGRATION_NON_AUTORISEE`     | constaté par le GUEST : le paquet booté dépasse le schéma des données sans `vault.migrer=1`, que le Worker ne pose que sous le geste « Mettre à jour l'application »                                                                                                                          | utiliser « Mettre à jour » s'il est proposé ; sinon signaler la publication                                               |
...
| privilégié                         | `vault.coquille.constater-le-dephasage`         | demande, lecture seule après le déverrouillage (#236 T2)         | 0042             |
| privilégié                         | `vault.coquille.constater-le-dephasage-reponse` | réponse : la décision publiée (issue, code, versions et schémas) | 0042             |
...
sur `sauvegarder-le-coffre-reponse` et `restaurer-le-coffre`, ADR 0039). `sansCapacite` refuse tout
le reste, et refuse ces deux-là sous tout autre nom, tout autre type et tout autre canal.
