...
export const MUTATIONS = Object.freeze([
  ...
  {
    nom: "le CLIQUET anti-DEK SUIT les alias de la porte jusqu'au point fixe",
    garde: "vm-cliquet-anti-dek — la propagation des noms APPRIS dans `aliasDeLaPorte`",
    fichier: CLIQUET,
    avant: "      for (const nom of liaisonsDuModule(code, connus).appris) connus.add(nom);",
    apres: "",
    epreuves: [CLIQUET],
  },
  {
    nom: "le CLIQUET anti-DEK suit un réexport EN BLOC de la porte",
    garde: "vm-cliquet-anti-dek — `propagerParReexportEnBloc`",
    fichier: CLIQUET,
    avant: "  propagerParReexportEnBloc(corpus, gestes);",
    apres: "",
    epreuves: [CLIQUET],
  },
  {
    nom: "le CLIQUET anti-DEK relève une référence OPAQUE à `importKey`",
    garde: "vm-cliquet-anti-dek — la conduite conservatrice de `referencesAImportKey`",
    fichier: CLIQUET,
    avant:
      "    aesGcm: opaques > 0 || appels.some((appel) => /AES-GCM|ALGORITHME_WEBCRYPTO/.test(appel)),",
    apres: "    aesGcm: appels.some((appel) => /AES-GCM|ALGORITHME_WEBCRYPTO/.test(appel)),",
    epreuves: [CLIQUET],
  },
  {
    nom: "le CLIQUET anti-DEK fait d'un RÉEXPORT une porte à part entière",
    garde: "vm-cliquet-anti-dek — `rendus.size > 0` dans `gestesDuModule`",
    fichier: CLIQUET,
    avant: "  const porte = definit || appelle || rendus.size > 0;",
    apres: "  const porte = definit || appelle;",
    epreuves: [CLIQUET],
  },
]);
